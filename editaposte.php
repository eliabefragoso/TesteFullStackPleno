...
<div class="content">
 ...
 <?php if(isset($_GET['id'])){ 
       $post = DB::getConn()->prepare("SELECT * FROM posts WHERE id=?;");
       $post->execute(array($_GET['id']));
       while($p = $post->fetch(PDO::FETCH_ASSOC)){
...
<form  enctype="multipart/form-data" action="actions/editarPost.php" class="pure-form pure-form-aligned" method="post">
 ...
 <p><label for="id_titulo">Titulo:</label> <input id="id_titulo" name="titulo" type="text" value="<?php echo $p['title'] ?>" /></p>   

  <p><label for="id_titulo">Slug:</label> <input id="id_slgu" name="slug" type="text" value="<?php echo $p['slug'] ?>" /></p>    

   <p><label for="id_titulo">Imagem:</label> <input id="id_titulo" name="imagem" type="file" /></p>          
                    
<p><label for="id_pergunta">Body:</label> <textarea cols="40" id="id_Body" name="body" rows="10"> <?php echo $p['body'] ?>
